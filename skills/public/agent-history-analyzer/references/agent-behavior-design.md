...
NEEDS_ONLINE_CONFIRMATION: before final implementation, refresh method references for
agent trajectory evaluation, ReAct-style trajectory analysis, Human-AI Interaction
Guidelines, and AI risk/trustworthiness frameworks.

Useful method references to re-check:
...
- `agent-behavior/report.md` must cite the confirmed model field mapping reference,
  such as `references/model-field-map.md`, including its version or date.
